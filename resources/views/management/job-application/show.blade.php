    ...
    <div class="p-4 space-y-8">
        ...
        @if ($jobApplication->interviews->isNotEmpty())
            <div class="max-w-[85rem] mx-auto">
                <x-tables.base-table header="Scheduled Interview ({{ $jobApplication->interviews->count() }})"
                    ...
                    @forelse($jobApplication->interviews->sortByDesc('date') as $key => $interview)
                        <tr class="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800 cursor-pointer">
                            ...
                            <x-tables.table-data>
                                <x-wireui-button outline label="Edit"
                                    href="{{ route('management.interview.edit', $interview) }}" target="_blank" />
                            </x-tables.table-data>
                        </tr>
                    @empty
